...
import 'package:yourappname/model/getcontentbychannelmodel.dart' as usercontent;
import 'package:yourappname/model/getcontentbychannelmodel.dart';
import 'package:yourappname/model/getreportreasonmodel.dart' as report;
...
import 'package:yourappname/model/shortmodel.dart' as shortlist;
import 'package:yourappname/model/watchlatermodel.dart' as watchlatershort;
  ...
  Future<void> getShortList(pageNo) async {
    ...
    if (shortModel.status == 200) {
      ...
      if (shortModel.result != null && (shortModel.result?.length ?? 0) > 0) {
        printLog("postModel length :==> ${(shortModel.result?.length ?? 0)}");
        int lastPosition = 0;
        ...
        printLog("shortVideoList length :==> ${(shortVideoList?.length ?? 0)}");
      }
    }
    loading = false;
    notifyListeners();
