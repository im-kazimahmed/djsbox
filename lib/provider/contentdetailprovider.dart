...
import 'package:yourappname/model/episodebyplaylistmodel.dart' as playlistdata;
...
/* Report Reason List Field */
  ...
  /* Pic Image into Image */
  PaletteGenerator? imageColor;
  ...
  fetchColorFromImage(PaletteGenerator color) {
    imageColor = color;
    notifyListeners();
  }

  isPlayIcon(String contentType, int index, isicon) {
  ...
  Future<void> getEpisodeByPodcast(podcastId, pageNo) async {
    ...
    if (epidoseByPodcastModel.status == 200) {
      ...
      if (epidoseByPodcastModel.result != null &&
        ...
        printLog('Now on page ==========> $podcastcurrentPage');
        if (epidoseByPodcastModel.result != null &&
            (epidoseByPodcastModel.result?.length ?? 0) > 0) {
          printLog(
              "followingModel length :==> ${(epidoseByPodcastModel.result?.length ?? 0)}");
